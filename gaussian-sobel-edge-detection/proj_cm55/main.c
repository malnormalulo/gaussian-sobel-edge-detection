...
CY_SECTION(".cy_itcm") int main(void)
{
    ...
    uint8_t *gauss_buffer = malloc(size * sizeof(uint8_t));
    perf_counter_start();
    gaussian_blur(IN_HEIGHT, IN_WIDTH, actual_out_monochrome, actual_out_gaussian_blur, gauss_buffer);
    res = perf_counter_stop();
